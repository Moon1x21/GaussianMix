...
class TrainerRICAP(Trainer):
    ...
    def ricap(self, images, targets):
        ...
        w,h = self.find_saliency(images[index[0]])
        ...
        for i in range(images.size(0)):
            ...
            for k in range(4):
                bbx1,bby1,bbx2,bby2 = self.saliency_bbox_rand(images[index[k]],w_[k],h_[k],k)
    ...
    def find_size(self,array, value,W,H):
        idx = np.unravel_index((np.abs(array- value)).argmin(),array.shape)
        w_array = array[:,idx[1]:idx[1]+1]
        w_idx = np.unravel_index(np.argmax(w_array, axis=None),w_array.shape)
        w = abs(idx[0]-w_idx[0])
        h_array = array[idx[0]:idx[0]+1,:]
        if w<16:
            w = w+16
        h_idx = np.unravel_index(np.argmax(h_array, axis=None),h_array.shape)
        h = abs(idx[1]-h_idx[1])
        if h<16:
            h = h+16
        return w, h
    
    def find_saliency(self,img):
        size = img.size()
        W = size[1]
        H = size[2]
        ...
        w,h = self.find_size(saliencyMap,np.median(saliencyMap, axis=None),W,H)

        return w,h         


    def saliency_bbox_rand(self,img,w_,h_,num):
        size = img.size()
        W = size[1]
        H = size[2]
        ...
        minV = np.min(saliencyMap,axis=None)

        if num == 0:
            rV = random.randrange(int(minV),int(midV)+1)
        else:
