...
class TrainerRICAP(Trainer):
    ...
    def find_size(self,array, value,W,H):
        ...
        w_mid = np.median(w_array, axis=None)
        w_idx = np.unravel_index((np.abs(w_array- w_mid)).argmin(),w_array.shape)
        w = abs(idx[0]-w_idx[0])
        if(w<=16):
            w = w+16
        h_array = array[idx[0]:idx[0]+1,:]
        h_mid = np.median(h_array, axis=None)
        h_idx = np.unravel_index((np.abs(h_array- h_mid)).argmin(),h_array.shape)
        h = abs(idx[1]-h_idx[1])
        if(h<=16):
            h = h+16
        return w, h
