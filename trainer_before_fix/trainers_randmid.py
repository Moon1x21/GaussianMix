...
class TrainerRICAP(Trainer):
    ...
    def find_nearest(self,array, value,W,w_,H,h_):
        ...
        sortarray = array
        sortarray = sortarray.reshape(-1,)
        sortarray = np.argsort(sortarray)
        m_idx = len(sortarray)
        mid = max((sortarray[m_idx//2],sortarray[m_idx//2-1]))
        idx = np.unravel_index((np.abs(array- mid)).argmin(),array.shape)
        return idx
