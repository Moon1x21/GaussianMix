    ...
    def ricap(self, images, targets):
        ...
        for k in range(4):
            ...
            for i in range(images.size(0)):
                bbx1,bby1,bbx2,bby2 = self.saliency_bbox(images[index[i]],w_[k],h_[k])
                cropped_images[k] = torch.cat((cropped_images[k],images[index[i]][:,bbx1:bbx2,bby1:bby2]),0)
            c_[k] = targets[index]
            W_[k] = (w_[k] * h_[k]) / (I_x * I_y)
            cropped_images[k] = torch.tensor(cropped_images[k])
        ...
        #         (torch.cat([cropped_images[0][i], cropped_images[1][i]], 1),
        #         torch.cat((cropped_images[2][i], cropped_images[3][i]), 1)),
        #         3),patched_images),0)
        ...
        #         bbx1,bby1,bbx2,bby2 = self.saliency_bbox(images[index[i]],w_[k],h_[k])
        #         if i == 0:
        #             img = torch.cat([images[index[i],:,bbx1:bbx2,bby1:bby2]],dim=0)
        #         else:
        #             img = torch.cat([img,images[index[i],:,bbx1:bbx2,bby1:bby2]],dim=0)
        #     cropped_images[k] = img
